...
public class ChatGptOcrProvider implements OcrProvider {
  ...
  public ChatGptOcrProvider(String apiKey, ImagePreprocessor imagePreprocessor) {
  ...
  public List<ParsedInventoryItem> parseInvoice(byte[] imageBytes) throws IOException {
    log.info("ChatGPT (gpt-4o-mini) invoice parse, image size before preprocess: {} bytes", imageBytes.length);
    ...
    log.info("Here is the thing: {}", body);
