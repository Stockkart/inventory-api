...
public class InvoicePdfService {
   ...
   * prints condensed at a hundred and twenty columns, and at the eighty-column size everything
   * past column eighty fell outside the preview pane and was simply not shown - the totals, the
   * right of the item grid and half the signature line all vanished, which read as the renderer
   * having dropped them.
   */
  private static String dotMatrixPreviewHtml(String text) {
    ...
    double fontPx = PREVIEW_BASE_FONT_PX
        * PREVIEW_BASE_COLUMNS / Math.max(PREVIEW_BASE_COLUMNS, widest);
    return "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"/>"
        + "<style>body{margin:0;padding:12px;background:#fff;}"
        + "pre{font-family:'Courier New',Courier,monospace;font-size:"
        + String.format(java.util.Locale.ROOT, "%.2f", fontPx) + "px;line-height:1.25;"
