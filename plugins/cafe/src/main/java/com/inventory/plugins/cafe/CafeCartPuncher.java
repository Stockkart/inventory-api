 ...
 *       still are</b>;
 *   <li>set every line's {@code kotSentQuantity} to its own {@code baseQuantity}, dropping the
...
public class CafeCartPuncher {
  ...
  /**
   * Per line: {@code baseQuantity - kotSentQuantity}, null-safe on both sides, keeping only the
  ...
  /**
   * Stage 2 — every line's {@code kotSentQuantity} becomes its own {@code baseQuantity}, then
  ...
  private static Document reconcileLinesStage() {
    ...
    // by the first: the $map above has just set kotSentQuantity to $ifNull(baseQuantity, 0), so
